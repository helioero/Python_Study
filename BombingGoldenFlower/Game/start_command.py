...
class Game(CardMethod):
    ...
    def __init__(self, user_count):
        ...
        self.user_init_card_list, self.user_point_and_color_card_list, self.user_color_card_list, self.user_size_card_list = self.get_cards(self.user_count)
        # 将牌的花色以及大小写入字典
    ...
    def user_input(self, head_amount, username):
        ...
        if info[username]['chips'] < head_amount:
            choose_borrow = input(
            ...
            if choose_borrow == '1':
                ...
                print(
                    f"玩家 {username} 弃牌, 剩余玩家 {info['not_drop_card_count']}")
                return 0
        while True:
            user_input_value = input(
                f"请输入下注的筹码，必须在区间({head_amount} - {info[username]['chips']}), 默认为 {head_amount}: ")
            if user_input_value == '':
                user_input_value = head_amount
                print(f'玩家下注 {head_amount}')
                break
            elif user_input_value.isdigit():
                if head_amount <= int(user_input_value) <= info[username]['chips']:
    ...
    def start(self):
        ...
        while True:
            ...
            print(f"所有玩家扣除底注 {info['low_chips']}")
            ...
            while True:
                ...
                for username in info['username_list']:
                    ...
                    if info['not_drop_card_count'] < 2:
                        info[username]['chips'] += info['chips_pool']
                        print(f"{'*' * 50}")
                        print(
                            f"{'*' * 2} 🎉🎉🎉 玩家 {username} 赢，赢得筹码 {info['chips_pool']} 🎉🎉🎉 {'*' * 2}")
                        print(f"{'*' * 50}")
                    ...
                    if view_card == '1':
                        ...
                        print(f"当前池子筹码为 {info['chips_pool']}")
                        ...
                        if choose_view_card == '1':
                            ...
                            print(
                                f"玩家 {username} 弃牌, 当前池子筹码为 {info['chips_pool']}, 剩余玩家 {info['not_drop_card_count']}")
                            continue
                        # 看牌后弃牌
                        else:
                    ...
                    if info['not_drop_card_count'] == 2 and info['unchecked_count'] == 0:
                        ...
                        if open_card == '1':
                            ...
                            if info[info['not_folded_list'][0]]['size_card'] > info[info['not_folded_list'][1]][
                                'size_card']:
                                info[info['not_folded_list'][0]]['chips'] += info['chips_pool']
                                print(f"{'*' * 50}")
                                print(
                                    f"**   玩家 {info['not_folded_list'][0]} 赢，赢得筹码 {info['chips_pool']}, 剩余 {info[info['not_folded_list'][0]]['chips']} 筹码  **")
                                print(f"{'*' * 50}")
                            else:
                                info[info['not_folded_list'][1]]['chips'] += info['chips_pool']
                                print(f"{'*' * 50}")
                                print(
                                    f"**   玩家 {info['not_folded_list'][1]} 赢，赢得筹码 {info['chips_pool']}, 剩余 {info[info['not_folded_list'][1]]['chips']} 筹码  **")
                                print(f"{'*' * 50}")
                            # 局数累加
                            print(f"{'-' * 50} 第 {round_count} 局结束 {'-' * 50}")
                            rounds_over = True
